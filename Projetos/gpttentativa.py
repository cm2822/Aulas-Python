...
def emprestimo_devolucao():
    ...
    livro['editora'] = input("Nome da Editora: ")
    livro['categoria'] = input("Qual categoria? ")
    livro['disponibilidade'] = input("Disponível? (s para sim, n para não) ").lower() == 's'

    livro_encontrado = next((livro for livro in livros if livro['isbn'].lower() == livro['isbn'].lower()), None)

    if livro_encontrado is not None:
        print(f"Nome do Livro: {livro_encontrado['titulo']}")
        ...
        if livro_encontrado['disponibilidade']:
            ...
            if emprestimo == 's':
                ...
                print(f"Empréstimo realizado com sucesso. Data do empréstimo: {datetime.datetime.now()}")
            else:
                devolucao = input("Deseja realizar a devolução deste livro? [s/n] ? --> ").lower()
                if devolucao == 's':
                    livro_encontrado['disponibilidade'] = True
                    print(f"Devolução realizada com sucesso. Data da devolução: {datetime.datetime.now()}")
        else:
